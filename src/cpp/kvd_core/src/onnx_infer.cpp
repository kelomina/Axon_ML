...
std::optional<OnnxModel> OnnxModel::load_from_file(const std::string& path) {
    try {
        ...
        auto session =
            std::make_unique<Ort::Session>(*env, wpath.c_str(), *options);
        ...
        m.impl_->allocator =
            std::make_unique<Ort::AllocatorWithDefaultOptions>();
...
std::optional<float> OnnxModel::predict_one(
    ...
    try {
        ...
        std::array<std::int64_t, 2> input_shape = {
            1, static_cast<std::int64_t>(input_size_)};
        ...
        session->Run(Ort::RunOptions{nullptr}, input_names.data(),
                     &input_tensor, 1, output_names.data(), &output_tensor, 1);
...
std::optional<std::vector<float>> OnnxModel::predict_batch(
    const std::vector<float>& features, std::size_t row_count,
    std::size_t num_features) const {
    if (!impl_ || !impl_->session) {
        return std::nullopt;
    }
    if (row_count == 0 || num_features != input_size_) {
        return std::nullopt;
    }
    ...
    try {
        ...
        std::array<std::int64_t, 2> output_shape = {
            static_cast<std::int64_t>(row_count),
...
bool OnnxModel::ok() const { return impl_ && impl_->session != nullptr; }

std::size_t OnnxModel::get_input_size() const { return input_size_; }

std::size_t OnnxModel::get_output_size() const { return output_size_; }
