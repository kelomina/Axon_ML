...
std::optional<OnnxModel> OnnxModel::load_from_file(const std::string& path) {
  try {
    Ort::Env* env = new Ort::Env(ORT_LOGGING_LEVEL_WARNING, "KoloVirusDetector");
    ...
    m.env_ = env;
    m.session_ = session;
    m.allocator_ = new Ort::AllocatorWithDefaultOptions();
    m.options_ = options;
...
std::optional<float> OnnxModel::predict_one(const std::vector<float>& features) const {
  ...
  try {
    ...
    Ort::AllocatorWithDefaultOptions* allocator = static_cast<Ort::AllocatorWithDefaultOptions*>(allocator_);
...
std::optional<std::vector<float>> OnnxModel::predict_batch(
  ...
  try {
    ...
    std::array<std::int64_t, 2> input_shape = {static_cast<std::int64_t>(row_count), static_cast<std::int64_t>(num_features)};
    std::array<std::int64_t, 2> output_shape = {static_cast<std::int64_t>(row_count), static_cast<std::int64_t>(output_size_)};
