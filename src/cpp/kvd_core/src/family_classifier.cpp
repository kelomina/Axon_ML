...
static float l2_distance_sq(const std::vector<float>& a,
...
static std::vector<float> apply_scaler(const std::vector<float>& x,
...
std::optional<FamilyClassifier> FamilyClassifier::load_from_json(
    ...
    try {
        ...
        fc.centroids_ =
            j.at("centroids").get<std::vector<std::vector<float>>>();
    ...
    if (fc.centroids_.empty() ||
        fc.centroids_.size() != fc.thresholds_.size() ||
        fc.centroids_.size() != fc.family_names_.size()) {
        return std::nullopt;
    }
    if (!fc.cluster_ids_.empty() &&
        fc.cluster_ids_.size() != fc.centroids_.size()) {
        return std::nullopt;
    }
    if (fc.cluster_ids_.empty()) {
...
std::optional<ScanResultFamily> FamilyClassifier::predict(
    ...
    ScanResultFamily r;
    r.cluster_id =
        cluster_ids_.empty() ? static_cast<int>(best_i) : cluster_ids_[best_i];
    r.family_name = family_names_[best_i];
    r.is_new_family = dist > thr;
    return r;
}

bool FamilyClassifier::ok() const { return !centroids_.empty(); }
