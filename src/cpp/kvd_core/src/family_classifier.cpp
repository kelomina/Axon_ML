...
static std::vector<float> apply_scaler(const std::vector<float>& x, const std::vector<float>& mean,
                                       const std::vector<float>& scale) {
    if (x.size() != mean.size() || x.size() != scale.size()) { return {}; }
    std::vector<float> y;
    y.resize(x.size());
...
std::optional<FamilyClassifier> FamilyClassifier::load_from_json(const std::string& path) {
    ...
    FamilyClassifier fc;
    try {
        if (j.contains("cluster_ids")) { fc.cluster_ids_ = j.at("cluster_ids").get<std::vector<int>>(); }
    ...
    if (fc.centroids_.empty() || fc.centroids_.size() != fc.thresholds_.size() ||
        fc.centroids_.size() != fc.family_names_.size()) {
        return std::nullopt;
    }
    if (!fc.cluster_ids_.empty() && fc.cluster_ids_.size() != fc.centroids_.size()) { return std::nullopt; }
    if (fc.cluster_ids_.empty()) {
        fc.cluster_ids_.resize(fc.centroids_.size());
        for (std::size_t i = 0; i < fc.cluster_ids_.size(); ++i) { fc.cluster_ids_[i] = static_cast<int>(i); }
    }
    return fc;
}

std::optional<ScanResultFamily> FamilyClassifier::predict(const std::vector<float>& features) const {
    if (centroids_.empty() || thresholds_.empty() || family_names_.empty()) { return std::nullopt; }
