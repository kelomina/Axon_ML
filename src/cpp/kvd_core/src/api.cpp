...
static std::shared_ptr<kvd::LightGbmModel> get_shared_model(
    const std::string& path);
static std::shared_ptr<kvd::FamilyClassifier> get_shared_family_classifier(
    const std::string& path);
...
kvd_handle* kvd_create(const kvd_config* config) {
    try {
        ...
        kvd::Config cfg = kvd::config_from_api(
            config->model_path, config->model_normal_path,
            config->model_packed_path, config->family_classifier_json_path,
            config->allowed_scan_root, config->max_file_size,
            config->prediction_threshold, config->onnx_model_path,
            config->onnx_model_normal_path, config->onnx_model_packed_path);

#if defined(KVD_SIGNATURE_ONLY)
        std::unique_ptr<kvd_handle> h(new (std::nothrow) kvd_handle{});
        if (!h) {
            return nullptr;
        }
        ...
        auto family_classifier =
            get_shared_family_classifier(cfg.family_classifier_json_path);
        std::unique_ptr<kvd_handle> h(new (std::nothrow) kvd_handle{});
        if (!h) {
            return nullptr;
        }
        ...
        if (!cfg.onnx_model_path.empty()) {
            ...
            if (onnx_m) {
                h->onnx_model =
                    std::make_shared<kvd::OnnxModel>(std::move(*onnx_m));
            }
        }
        if (!cfg.onnx_model_normal_path.empty()) {
            auto onnx_m_normal =
                kvd::OnnxModel::load_from_file(cfg.onnx_model_normal_path);
            if (onnx_m_normal) {
                h->onnx_model_normal =
                    std::make_shared<kvd::OnnxModel>(std::move(*onnx_m_normal));
            }
        }
        if (!cfg.onnx_model_packed_path.empty()) {
            auto onnx_m_packed =
                kvd::OnnxModel::load_from_file(cfg.onnx_model_packed_path);
            if (onnx_m_packed) {
                h->onnx_model_packed =
                    std::make_shared<kvd::OnnxModel>(std::move(*onnx_m_packed));
            }
        }
...
static int write_json_out(const nlohmann::json& j, char** out_json,
                          size_t* out_len) {
    if (!out_json || !out_len) {
        return -1;
    }
...
static int write_string_out(const std::string& s, char** out_json,
                            size_t* out_len) {
    if (!out_json || !out_len) {
        return -1;
    }
...
static int write_train_error(const std::string& code, char** out_json,
                             size_t* out_len) {
    nlohmann::json j;
    j["ok"] = false;
    j["total"] = 0;
...
static int validate_hardcase_manifest(const std::string& manifest_path,
    ...
    for (const auto& item : model_paths) {
        ...
        std::string model_path_utf8(reinterpret_cast<const char*>(u8.data()),
                                    u8.size());
        std::string runtime_model_path =
            resolve_lightgbm_runtime_path(model_path_utf8);
...
static void collect_train_targets(const std::string& path,
    ...
    for (; it != end; it.increment(ec)) {
        ...
        if (it->is_regular_file(ec)) {
            ...
            out.push_back(std::string(reinterpret_cast<const char*>(u8.data()),
                                      u8.size()));
        }
    }
}

static int kvd_train_internal(kvd_handle* handle,
                              const std::vector<std::string>& paths,
                              bool is_malware, char** out_json,
                              size_t* out_len) {
    if (!handle) {
        return -1;
    }
...
static std::shared_ptr<kvd::LightGbmModel> get_shared_model(
    ...
    static std::mutex mu;
    static std::unordered_map<std::string, std::weak_ptr<kvd::LightGbmModel>>
        cache;
...
static std::shared_ptr<kvd::FamilyClassifier> get_shared_family_classifier(
    ...
    static std::mutex mu;
    static std::unordered_map<std::string, std::weak_ptr<kvd::FamilyClassifier>>
        cache;
    std::lock_guard<std::mutex> lock(mu);
...
struct TokenHash {
    std::size_t operator()(const std::weak_ptr<int>& p) const noexcept {
        auto sp = p.lock();
        if (!sp) return 0;
        return std::hash<int*>()(sp.get());
    }
};

struct TokenEq {
    bool operator()(const std::weak_ptr<int>& a,
                    const std::weak_ptr<int>& b) const noexcept {
        auto sp_a = a.lock();
        auto sp_b = b.lock();
        if (!sp_a || !sp_b) return false;
        return sp_a.get() == sp_b.get();
    }
};
...
static kvd::MalwareScanner* get_thread_scanner(kvd_handle* handle) {
    ...
    thread_local std::unordered_map<std::weak_ptr<int>,
                                    std::unique_ptr<kvd::MalwareScanner>,
                                    TokenHash, TokenEq>
        cache;
    ...
    std::weak_ptr<int> weak_token = handle->token;
    auto it = cache.find(weak_token);
    if (it != cache.end()) {
        return it->second.get();
    }
    auto scanner_opt = kvd::MalwareScanner::create_shared(
        handle->config, handle->model, handle->model_normal,
        handle->model_packed, handle->family_classifier, handle->onnx_model,
        handle->onnx_model_normal, handle->onnx_model_packed);
    if (!scanner_opt) {
        return nullptr;
    }
    auto ptr = std::make_unique<kvd::MalwareScanner>(std::move(*scanner_opt));
    auto raw = ptr.get();
    cache.emplace(weak_token, std::move(ptr));
    return raw;
}

int kvd_scan_path(kvd_handle* handle, const char* path, char** out_json,
                  size_t* out_len) {
    try {
        if (!handle || !path) {
            return -1;
        ...
        kvd_log_event("kvd_scan_path_exception",
                      kvd_basename(path) + ":" + e.what());
        return -1;
    } catch (...) {
        kvd_log_event("kvd_scan_path_exception",
                      kvd_basename(path) + ":unknown");
        return -1;
    }
}

int kvd_scan_bytes(kvd_handle* handle, const unsigned char* bytes, size_t len,
...
int kvd_scan_paths(kvd_handle* handle, const char** paths, size_t count,
                   char** out_json, size_t* out_len) {
    try {
        if (!handle) {
            return -1;
        ...
        kvd_log_event("kvd_scan_paths_exception",
                      std::to_string(count) + ":" + e.what());
        return -1;
    } catch (...) {
        kvd_log_event("kvd_scan_paths_exception",
                      std::to_string(count) + ":unknown");
        return -1;
    }
}

int kvd_train_path(kvd_handle* handle, const char* path, int label,
                   char** out_json, size_t* out_len) {
    try {
        if (!handle || !path) {
        ...
        kvd_log_event("kvd_train_path_exception",
                      kvd_basename(path) + ":" + e.what());
...
int kvd_train_paths(kvd_handle* handle, const char** paths, size_t count,
                    int label, char** out_json, size_t* out_len) {
    try {
        if (!handle) {
            return write_train_error("invalid_argument", out_json, out_len);
        }
        ...
        kvd_log_event("kvd_train_paths_exception",
                      std::to_string(count) + ":" + e.what());
        return write_train_error("exception", out_json, out_len);
    } catch (...) {
        kvd_log_event("kvd_train_paths_exception",
...
int kvd_train_from_path(kvd_handle* handle, const char* path, int label,
                        char** out_json, size_t* out_len) {
...
void kvd_free(char* p) { std::free(p); }

int kvd_extract_pe_features(const char* path, float* out_features,
                            size_t out_len) {
    try {
        ...
        std::vector<float> features =
            kvd::extract_combined_pe_features_from_path(path, std::nullopt);
        ...
        kvd_log_event("kvd_extract_pe_features_exception",
                      kvd_basename(path) + ":" + e.what());
        return -2;
    } catch (...) {
        kvd_log_event("kvd_extract_pe_features_exception",
...
int kvd_extract_pe_features_batch(const char** paths, size_t count,
                                  float* out_features, size_t feature_dim,
    ...
    try {
        ...
        std::size_t workers =
            thread_count > 0 ? static_cast<std::size_t>(thread_count) : hw;
        if (workers == 0) {
            workers = 1;
        }
        ...
        for (std::size_t t = 0; t < workers; ++t) {
            pool.emplace_back([&]() {
                for (;;) {
                    ...
                    try {
                        std::vector<float> features =
                            kvd::extract_combined_pe_features_from_path(
                                p, std::nullopt);
                        if (features.size() != KVD_PE_FEATURE_VECTOR_DIM) {
                            out_status[i] = -2;
                            continue;
...
size_t kvd_get_pe_feature_dimension(void) { return KVD_PE_FEATURE_VECTOR_DIM; }

int kvd_validate_models(const kvd_config* config, char** out_error,
                        size_t* out_len) {
    try {
        if (!config) {
        ...
        std::string runtime_model_path =
            resolve_lightgbm_runtime_path(cfg.model_path);
        if (!path_exists(runtime_model_path)) {
            int rc = write_error("model_main_missing");
            return rc == 0 ? KVD_MODEL_ERR_MAIN_MISSING : rc;
        }
        ...
        if (has_normal) {
            std::string runtime_model_normal_path =
                resolve_lightgbm_runtime_path(cfg.model_normal_path);
            ...
            if (!kvd::LightGbmModel::load_from_file(
                    runtime_model_normal_path)) {
        ...
        if (has_packed) {
            std::string runtime_model_packed_path =
                resolve_lightgbm_runtime_path(cfg.model_packed_path);
        ...
        if (!cfg.family_classifier_json_path.empty()) {
            ...
            if (!kvd::FamilyClassifier::load_from_json(
                    cfg.family_classifier_json_path)) {
        ...
        if (!cfg.hardcase_manifest_path.empty()) {
            ...
            int hardcase_rc = validate_hardcase_manifest(
                cfg.hardcase_manifest_path, hardcase_error);
