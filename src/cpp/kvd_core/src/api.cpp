...
static nlohmann::json to_json(const kvd::ScanResult& r) {
    ...
    if (!r.hardcase_class.empty()) { j["hardcase_class"] = r.hardcase_class; }
    if (!r.hardcase_scores.empty()) { j["hardcase_scores"] = r.hardcase_scores; }
    j["signature_hit"] = r.signature_hit;
    j["signature_score"] = r.signature_score;
    if (!r.signature_reason.empty()) { j["signature_reason"] = r.signature_reason; }
...
kvd_handle* kvd_create(const kvd_config* config) {
    try {
        ...
        kvd::Config cfg =
            kvd::config_from_api(config->model_path, config->model_normal_path, config->model_packed_path,
                                 config->family_classifier_json_path, config->allowed_scan_root, config->max_file_size,
                                 config->prediction_threshold, config->onnx_model_path, config->onnx_model_normal_path,
        ...
        if (!cfg.onnx_model_path.empty()) {
            ...
            if (onnx_m) { h->onnx_model = std::make_shared<kvd::OnnxModel>(std::move(*onnx_m)); }
        }
        if (!cfg.onnx_model_normal_path.empty()) {
            auto onnx_m_normal = kvd::OnnxModel::load_from_file(cfg.onnx_model_normal_path);
            if (onnx_m_normal) { h->onnx_model_normal = std::make_shared<kvd::OnnxModel>(std::move(*onnx_m_normal)); }
        }
        if (!cfg.onnx_model_packed_path.empty()) {
            auto onnx_m_packed = kvd::OnnxModel::load_from_file(cfg.onnx_model_packed_path);
            if (onnx_m_packed) { h->onnx_model_packed = std::make_shared<kvd::OnnxModel>(std::move(*onnx_m_packed)); }
        }
...
void kvd_destroy(kvd_handle* handle) {
    try {
        if (!handle) { return; }
        delete handle;
    } catch (const std::exception& e) { kvd_log_event("kvd_destroy_exception", e.what()); } catch (...) {
...
static std::string resolve_lightgbm_runtime_path(const std::string& path) {
    ...
    if (!fs_opt) { return path; }
    const auto& fs_path = *fs_opt;
    if (!is_onnx_path(fs_path)) { return path; }
...
static int validate_hardcase_manifest(const std::string& manifest_path, std::string& error_code) {
    ...
    for (const auto& item : model_paths) {
        ...
        if (model_path.is_relative()) { model_path = base_dir / model_path; }
...
static int kvd_train_internal(kvd_handle* handle, const std::vector<std::string>& paths, bool is_malware,
        ...
        for (std::size_t t = 0; t < threads; ++t) {
            pool.emplace_back([&]() {
                ...
                for (;;) {
                    ...
                    if (scanner->train_path(files[i], is_malware)) { trained.fetch_add(1, std::memory_order_relaxed); }
                }
            });
        }
...
struct TokenEq {
    bool operator()(const std::weak_ptr<int>& a, const std::weak_ptr<int>& b) const noexcept {
        auto sp_a = a.lock();
        auto sp_b = b.lock();
...
static kvd::MalwareScanner* get_thread_scanner(kvd_handle* handle) {
    if (!handle || !handle->token) { return nullptr; }
    thread_local std::unordered_map<std::weak_ptr<int>, std::unique_ptr<kvd::MalwareScanner>, TokenHash, TokenEq> cache;
    ...
    auto scanner_opt = kvd::MalwareScanner::create_shared(
        handle->config, handle->model, handle->model_normal, handle->model_packed, handle->family_classifier,
        handle->onnx_model, handle->onnx_model_normal, handle->onnx_model_packed);
    if (!scanner_opt) { return nullptr; }
...
int kvd_train_path(kvd_handle* handle, const char* path, int label, char** out_json, size_t* out_len) {
    try {
        if (!handle || !path) { return write_train_error("invalid_argument", out_json, out_len); }
        std::vector<std::string> v;
        v.emplace_back(path);
...
int kvd_train_paths(kvd_handle* handle, const char** paths, size_t count, int label, char** out_json, size_t* out_len) {
    try {
        if (!handle) { return write_train_error("invalid_argument", out_json, out_len); }
        if (!paths && count > 0) { return write_train_error("invalid_argument", out_json, out_len); }
        std::vector<std::string> v;
        v.reserve(count);
        for (size_t i = 0; i < count; ++i) {
            if (paths && paths[i]) {
...
int kvd_extract_pe_features(const char* path, float* out_features, size_t out_len) {
    try {
        if (!path || !out_features) { return -1; }
        if (out_len < KVD_PE_FEATURE_VECTOR_DIM) { return -1; }
        std::vector<float> features = kvd::extract_combined_pe_features_from_path(path, std::nullopt);
        if (features.size() != KVD_PE_FEATURE_VECTOR_DIM) { return -2; }
...
int kvd_extract_pe_features_batch(const char** paths, size_t count, float* out_features, size_t feature_dim,
                                  int* out_status, unsigned int thread_count) {
    try {
        if (!paths || !out_features || !out_status) { return -1; }
        if (feature_dim < KVD_PE_FEATURE_VECTOR_DIM) { return -1; }
        if (count == 0) { return 0; }
...
int kvd_validate_models(const kvd_config* config, char** out_error, size_t* out_len) {
    try {
        ...
        if ((out_error && !out_len) || (!out_error && out_len)) { return KVD_MODEL_ERR_INVALID_ARGUMENT; }
