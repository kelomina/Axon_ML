...
std::optional<LightGbmModel> LightGbmModel::load_from_file(
    const std::string& path) {
    void* booster = nullptr;
    int num_iterations = 0;
    if (LGBM_BoosterCreateFromModelfile(path.c_str(), &num_iterations,
                                        &booster) != 0) {
        return std::nullopt;
    }
    LightGbmModel m;
...
std::optional<float> LightGbmModel::predict_one(
    ...
    double out = 0.0;
    int64_t out_len = 0;
    if (LGBM_BoosterPredictForMat(handle_, features.data(), C_API_DTYPE_FLOAT32,
                                  1, static_cast<int>(features.size()), 1,
                                  C_API_PREDICT_NORMAL, 0, -1, "", &out_len,
                                  &out) != 0) {
        return std::nullopt;
    }
    if (out_len < 1) {
...
std::optional<std::vector<float>> LightGbmModel::predict_batch(
    ...
    if (LGBM_BoosterPredictForMat(
            handle_, features.data(), C_API_DTYPE_FLOAT32,
            static_cast<int>(row_count), static_cast<int>(num_features), 1,
            C_API_PREDICT_NORMAL, 0, -1, "", &out_len, out.data()) != 0) {
        return std::nullopt;
    }
...
bool LightGbmModel::ok() const { return handle_ != nullptr; }
