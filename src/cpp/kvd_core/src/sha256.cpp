...
std::uint8_t sha256_last_byte_utf8(const std::string& s) {
    ...
    if (BCryptGetProperty(alg, BCRYPT_OBJECT_LENGTH,
                          reinterpret_cast<PUCHAR>(&hash_object_len),
    ...
    if (BCryptCreateHash(alg, &hash, hash_object.data(), hash_object_len,
                         nullptr, 0, 0) != 0) {
        BCryptCloseAlgorithmProvider(alg, 0);
        return 0;
    }
    ...
    if (!s.empty()) {
        if (BCryptHashData(
                hash, reinterpret_cast<PUCHAR>(const_cast<char*>(s.data())),
                static_cast<ULONG>(s.size()), 0) != 0) {
