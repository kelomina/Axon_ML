...
TEST(kvd_smoke, create_null_config) {
...
TEST(kvd_smoke, create_with_model_path) {
...
TEST(kvd_smoke, scan_path_on_current_exe) {
  auto root = find_repo_root();
  ASSERT_FALSE(root.empty());
  auto model_path = (root / "saved_models" / "lightgbm_model.txt").string();
  auto family_path = (root / "hdbscan_cluster_results" / "family_classifier.json").string();
...
TEST(kvd_smoke, dll_exports_present) {
#if defined(_WIN32)
...
TEST(kvd_smoke, scan_path_multithread) {
  auto root = find_repo_root();
  ASSERT_FALSE(root.empty());
  auto model_path = (root / "saved_models" / "lightgbm_model.txt").string();
  auto family_path = (root / "hdbscan_cluster_results" / "family_classifier.json").string();
