...
int main(int argc, char** argv) {
  ...
  if (dll_path.empty()) dll_path = "signature_engine.dll";

  std::string target = get_arg_value(argc, argv, "--target");
  if (target.empty()) target = current_exe_path();
  if (target.empty()) {
    std::cerr << "target is empty\n";
    return 2;
  }
