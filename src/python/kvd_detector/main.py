...
class _EmbeddedImporter(importlib.abc.MetaPathFinder, importlib.abc.Loader):
    ...
    def find_spec(self, fullname, path=None, target=None):
        if fullname in _EMBEDDED_SOURCES:
            return importlib.util.spec_from_loader(
                fullname, self, is_package=fullname in _EMBEDDED_PACKAGES
            )
        return None
    ...
    def _validate_source(self, source, module_name):
        ...
        if module_name.startswith(
            ...
            dangerous_patterns = tuple(
                p for p in dangerous_patterns if p != "__import__("
            )
    ...
    def exec_module(self, module):
        ...
        if not self._ALLOWED_MODULE_PREFIXES or any(
            ...
            if self._validate_source(source, module.__name__):
                ...
                raise RuntimeError(
                    f"Embedded source for {module.__name__} failed security validation"
                )
...
_register_embedded(
    "training.export_family_classifier_json",
    r"""import argparse
import json
import pickle
...
_register_embedded(
    "validation.feature_gating_experiment",
    r"""import os
import json
import argparse
import numpy as np
...
_register_embedded(
    "training.hardcase_model_trials",
    r"""import json
from pathlib import Path
import numpy as np
import pandas as pd
